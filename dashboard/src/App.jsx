...
// Import CSS in index.html: <link href="https://api.mapbox.com/mapbox-gl-js/v3.3.0/mapbox-gl.css" rel="stylesheet">
...
function SOSBanner({ activeEvent, onView }) {
  ...
  const tc = threatColor(activeEvent.threat_level || "HIGH");
...
function LiveMap({ breadcrumbs, activeEvent }) {
  ...
  useEffect(() => {
    if (!mapRef.current || !breadcrumbs?.length) return;
    const map = mapRef.current;

    const coords = breadcrumbs.map(b => [b.lon, b.lat]);
    ...
    const ready = () => {
      ...
      if (markerRef.current) {
        ...
        `;
        markerRef.current = new mapboxgl.Marker(el).setLngLat(last).addTo(map);
      }

      map.flyTo({ center: last, zoom: 15, duration: 1000 });
    };
  ...
  return (
    <div style={{ position: "relative", borderRadius: 12, overflow: "hidden", flex: 1, minHeight: 380 }}>
      <div ref={mapContainer} style={{ width: "100%", height: "100%" }} />
      {!MAPBOX_TOKEN && (
        <div style={{
          position: "absolute", inset: 0, background: "#f1f5f9",
          display: "flex", alignItems: "center", justifyContent: "center",
          flexDirection: "column", gap: 8, color: "#64748b", fontSize: 13,
        }}>
          <span style={{ fontSize: 32 }}>🗺️</span>
          <div>Set VITE_MAPBOX_TOKEN to enable live map</div>
          {activeEvent && (
              ...
              {aiData ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  <div style={{
                    ...
                    background: threatColor(aiData.threat_level).bg,
                    border: `1.5px solid ${threatColor(aiData.threat_level).dot}`,
                  }}>
                    <div style={{ fontWeight: 800, fontSize: 13, color: threatColor(aiData.threat_level).text, marginBottom: 8 }}>
                      THREAT LEVEL: {aiData.threat_level}
                    </div>
                    <p style={{ margin: 0, color: "#1e293b", lineHeight: 1.6 }}>{aiData.summary}</p>
                  </div>

                  {aiData.tone_analysis && (
                    <div style={{ padding: 16, background: "#f8fafc", borderRadius: 12, border: "1px solid #e2e8f0" }}>
                      <div style={{ fontWeight: 700, fontSize: 12, color: "#64748b", marginBottom: 8 }}>TONE ANALYSIS</div>
                      <p style={{ margin: 0, color: "#334155" }}>{aiData.tone_analysis}</p>
                    </div>
                  )}
                  ...
                  {aiData.recommended_action && (
                    <div style={{
                      ...
                      <p style={{ margin: 0, color: "#78350f", fontWeight: 600 }}>{aiData.recommended_action}</p>
                    </div>
                  )}

                  {aiData.key_observations?.length > 0 && (
                    <div style={{ padding: 16, background: "#f0f9ff", borderRadius: 12, border: "1px solid #bae6fd" }}>
                      <div style={{ fontWeight: 700, fontSize: 12, color: "#075985", marginBottom: 10 }}>KEY OBSERVATIONS</div>
                      <ul style={{ margin: 0, paddingLeft: 20, color: "#0c4a6e" }}>
                        {aiData.key_observations.map((obs, i) => (
